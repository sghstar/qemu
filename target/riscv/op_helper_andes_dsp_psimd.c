...
static int64_t
get_udouble (uint64_t even, uint64_t odd)
{
    return (odd << 32) | (even& 0xFFFFFFFF);
}


static int64_t
get_double (int64_t even, int64_t odd)
{
    return (odd << 32) | (even & 0xFFFFFFFF);
}

static uint32_t get_ueven(uint64_t val)
{
      return  val & 0xFFFFFFFF;
}
static uint32_t get_uodd(uint64_t val)
{
      return (val >> 32) & 0xFFFFFFFF;
}
static int32_t get_even(int64_t val)
{
      return  val & 0xFFFFFFFF;
}
static int32_t get_odd(int64_t val)
{
      return (val >> 32) & 0xFFFFFFFF;
}
...
uint64_t psimd_rrr64(uint32_t op, uint64_t rs1, uint64_t rs2) {
  ...
  reg_rs2.u = rs2;
    ...
    case radd64:
      {
        union64_t a, b, t;
        union32u_t a_index, b_index, t_index;
        int32_t lsb_eq_1 = 1;
